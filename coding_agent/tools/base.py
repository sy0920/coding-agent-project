...
class Tool:
    ...
    # 风险等级，决定人工审批策略：
    #   readonly  = 只读、无副作用，永不审批；
    #   modify    = 会改动文件，仅在 --approve 下审批；
    #   dangerous = 任意命令执行，默认就审批。
    risk: str = "readonly"
...
class ToolRegistry:
    ...
    def risk_of(self, name: str) -> str:
        """返回工具的风险等级（readonly / modify / dangerous）；未知工具按只读处理。"""
        tool = self._tools.get(name)
        return tool.risk if tool else "readonly"
