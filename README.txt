...
4. 交互模式：python -m coding_agent（多轮会话、每轮自动保存，支持 /save /resume /sessions /clear）
5. 常用参数：--workspace 指定工作目录；--max-iterations 限制迭代轮数；--verbose 打印过程；--approve 危险操作前审批
6. 运行测试：python -m pytest（离线，不消耗 API）

二、特色功能
1. 完整 agent 循环：思考—行动—观察，直到产出最终回答或调用 finish 工具。
2. 会话管理：多轮会话记忆 + 持久化（每轮自动保存；/save、/resume、/switch、/sessions、/clear）。
3. 自研工具系统：9 个工具（读写/替换文件、列目录、搜索、执行命令、git、todo、finish），全部本地执行。
4. 任务规划：todo 工具让模型先列计划、完成后更新。
5. 人工审批：--approve 开启后，执行命令等危险操作前询问确认。
